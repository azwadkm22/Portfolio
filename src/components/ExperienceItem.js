...
function ExperienceItem({ company, title, date, description }) {
    return (
        <div className="border-l-4 border-blue-500 pl-6 mb-10">
            ...
            <p className="text-sm md:text-md text-gray-500 mb-1">{date}</p>
            <p className="text-sm md:text-xl font-medium text-gray-700">{title}</p>
            <p className="text-sm md:text-lg text-gray-600 mt-3">{description}</p>
        </div>
    );
}
